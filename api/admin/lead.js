...
module.exports = async function handler(request, response) {
  ...
  if (!ids.length) {
    return sendJson(response, 400, { error: "Choose at least one lead." });
  }
  ...
  try {
    if (request.method === "DELETE") {
      ...
      for (const id of ids) {
        deleted.push(await deleteLeadById(id));
      }
    ...
    for (const id of ids) {
      updated.push(await updateLeadById(id, patch, session.sub));
    }

    return sendJson(response, 200, { ok: true, leads: updated });
  } catch (error) {
    console.error("SAMER_SOLUTIONS_ADMIN_LEAD_WRITE_ERROR", error);
    return sendJson(response, error.status || 500, {
      error: error.status ? error.message : "Could not update this lead.",
